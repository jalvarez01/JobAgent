...
def guardar_favorito(data: FavoritoCreate, svc: FavoritoService = Depends(_service)):
    favorito, es_nuevo = svc.guardar_favorito(data.perfil_id, data.vacante_id)
    return favorito


@router.get("/perfil/{perfil_id}", response_model=list[FavoritoResponse])
def listar_favoritos(perfil_id: str, svc: FavoritoService = Depends(_service)):
    return svc.listar_favoritos(perfil_id)


@router.get("/check/{perfil_id}/{vacante_id}")
def es_favorito(perfil_id: str, vacante_id: str, svc: FavoritoService = Depends(_service)):
    return {"es_favorito": svc.es_favorito(perfil_id, vacante_id)}


@router.delete("/{perfil_id}/{vacante_id}", status_code=204)
def eliminar_favorito(perfil_id: str, vacante_id: str, svc: FavoritoService = Depends(_service)):
    eliminado = svc.eliminar_favorito(perfil_id, vacante_id)
    if not eliminado:
